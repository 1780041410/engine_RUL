...
def cycle_level_2_unit_level(cycle_level_prediction,unit_cycle_ID_test):       
    ...
    unit_ID = unit_cycle_ID_test.loc[:,index_unit]      

    now_age_FD001 = pd.read_table('now_age_FD001.txt',delim_whitespace=True,header=None,names=index_age,encoding='utf-8')  
    cycle_based_pred_results_with_ID = pd.concat([unit_cycle_ID_test,cycle_based_pred_results],axis=1)

    total_cycles = cycle_based_pred_results_with_ID.cycle + cycle_based_pred_results_with_ID.rul       

    total_cycles_with_ID = pd.concat([unit_ID,total_cycles],axis=1)     

    last_unit = int(max(unit_ID.unit))         
